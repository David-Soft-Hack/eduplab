        ...
        {stats.map((stat, idx) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className={cn("p-3 rounded-2xl text-white group-hover:scale-110 transition-transform", stat.color)}>
                <stat.icon size={22} />
              </div>
            </div>
            <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">{stat.label}</h3>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-3xl font-display font-bold text-slate-800">{stat.value}</span>
              <span className="text-[10px] font-bold text-emerald-500 uppercase">{stat.trend}</span>
            </div>
          </motion.div>
        ))}
      </div>
            ...
            {todaysSessions.length > 0 ? (
              todaysSessions.map((session, idx) => (
                <motion.div
                  ...
                  transition={{ delay: idx * 0.1 }}
                  className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6 md:items-center hover:shadow-lg transition-all border-l-4 border-l-academic-500"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="px-3 py-1 bg-academic-50 text-academic-600 rounded-lg text-[10px] font-bold uppercase tracking-wider">{session.grupo}</span>
                      <span className="text-xs font-bold text-slate-300">•</span>
                      <span className="text-xs font-bold text-slate-400">{session.carrera}</span>
                      <span className="text-xs font-bold text-slate-300">•</span>
                      <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md uppercase">{session.turno}</span>
                    </div>
                    <h3 className="text-2xl font-bold text-slate-800 leading-tight">{session.moduloNombre}</h3>
                    <div className="flex flex-wrap gap-4 mt-4">
                      {session.actividades.map((act: any) => (
                        <div key={act.id} className="flex items-center gap-2">
                          <div className="w-1.5 h-1.5 rounded-full bg-academic-400" />
                          <span className="text-sm font-medium text-slate-600 truncate max-w-[200px]">{act.desc}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="flex items-center gap-6 shrink-0">
                    <div className="text-right">
                      <div className="flex items-center gap-2 justify-end text-academic-600 font-black text-xl">
                        <Clock size={20} />
                        {session.horas}h
                      </div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Carga Horaria</p>
                    </div>
                    <button 
                      onClick={() => {
                      ...
                      className="w-14 h-14 bg-academic-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-academic-600/20 hover:bg-academic-700 transition-all active:scale-95 group"
                    >
